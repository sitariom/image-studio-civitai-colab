...
# ============================================================================
def load_lora_from_civitai(url_or_id, token, weight=1.0):
...
def load_vae_from_civitai(url_or_id, token):
    try:
        model_id = parse_civitai_id(url_or_id)
        if not model_id:
...
def load_ti_from_civitai(url_or_id, token):
...
with gr.Blocks(theme=gr.themes.Soft(), css=CSS, title="Advanced Multi-Model Image Studio (Definitivo v2)") as demo:
    ...
    with gr.Tabs():
        with gr.TabItem("Modelo / Configuracao"):
            ...
            gr.Markdown("---")
            gr.Markdown("### LoRA / VAE / TextualInversion (Civitai) — baixa e ativa automaticamente")
            with gr.Row():
    ...
    lora_btn.click(fn=load_lora_from_civitai, inputs=[lora_url, civitai_token, lora_weight], outputs=[lora_status])
